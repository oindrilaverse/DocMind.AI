...
import { Brain, Lock, Mail, User, AlertTriangle } from 'lucide-react';
...
export const RegisterPage: React.FC = () => {
  ...
  const getErrorMessage = () => {
    ...
    if (registerError) {
      return (registerError as any).response?.data?.message || 'Email is already registered';
    }
    return '';
  };
    ...
    <div className="min-h-screen bg-darkbg flex flex-col justify-center items-center px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="w-full max-w-md z-10">
        ...
        <div className="glass-panel rounded-3xl p-8 shadow-2xl relative">
          <h2 className="text-xl font-semibold text-textmain mb-6">Create Account</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label htmlFor="name" className="block text-xs font-semibold uppercase tracking-wider text-textmuted mb-2">
                Full Name
              </label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none">
                  <User className="h-5 h-5 text-textmuted" />
                </span>
                <input
                  id="name"
                  type="text"
            ...
            <div>
              <label htmlFor="confirmPassword" className="block text-xs font-semibold uppercase tracking-wider text-textmuted mb-2">
                Confirm Password
              </label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none">
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-textmuted">
              ...
              <Link to="/login" className="text-primary hover:underline font-semibold transition duration-150">
                Sign In
              </Link>
            </p>
          </div>
